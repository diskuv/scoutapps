...
module Arguments : sig
  ...
  let set reference = Arg.Bool (fun value -> reference := Some value)

  let args =
    [
      ("--use-libev", set use_libev, "BOOLEAN whether to check for libev");
      ( "--use-pthread",
        set use_pthread,
      ...
      ("--verbose", Arg.Set verbose, "BOOLEAN show results of feature detection");
    ]
  ...
  let parse arguments =
    try
      Arg.parse_argv ~current:(ref 0)
        ...
        (Printf.sprintf "Environment variable usage: %s=[OPTIONS]"
...
module C_library_flags : sig
  ...
  let add_link_flags flags = extend [] flags
  let ( // ) = Filename.concat

  let libev_search_path =
    match {|@libev_INSTALL_DIR@|} with "" -> [] | p -> [ p ]

  let default_search_paths =
    (* Search first in CMake *)
    libev_search_path
    @ [
        "/usr";
        (* No homebrew; no unreproducible, non-system dependencies. "/usr/local"; *)
        "/usr/pkg";
        "/opt";
        (* No unreproducible, non-system dependencies. "/opt/local"; *)
        "/sw";
        "/mingw";
      ]
  ...
  let paths_from_environment_variable variable =
    ...
    | exception Not_found -> []
    | paths ->
        Configurator.Flags.extract_words paths
          ~is_word_char:(( <> ) path_separator)
        |> List.map Filename.dirname

  let search_paths =
    lazy
      (* No unreproducible environment variables:
  ...
  let default argument fallback =
    match argument with Some value -> value | None -> fallback
  ...
  let detect ?env_var ?package ?header context ~library =
    ...
    | None -> (
        let flags_from_pkg_config =
          ...
          | None -> None
          | Some pkg_config -> Configurator.Pkg_config.query pkg_config ~package
        in

        match flags_from_pkg_config with
        | Some flags -> extend flags.cflags flags.libs
        | None -> (
            try
              let path =
                List.find
...
module Output : sig
  ...
  let write_ml_file ?(extra = []) macros =
    macros
    |> List.map (fun { name; found } ->
           Printf.sprintf "let _%s = %b" name found)
    |> ( @ )
         (List.map
            (fun { name; found } -> Printf.sprintf "let %s = %b" name found)
            extra)
    |> Configurator.Flags.write_lines ml_file

  let write_flags_files () =
    Configurator.Flags.write_sexp c_flags_file (C_library_flags.c_flags ());
    Configurator.Flags.write_sexp link_flags_file
      (C_library_flags.link_flags ())
end
...
module Features : sig
  ...
  let features = ref []
  let feature the_feature = features := !features @ [ the_feature ]

  let verbose =
    Printf.ksprintf (fun s -> if !Arguments.verbose then print_string s)
  ...
  let compiles ?(werror = false) ?(link_flags = []) context code =
    ...
    let c_flags = if werror then "-Werror" :: c_flags else c_flags in
    let link_flags = link_flags @ C_library_flags.link_flags () in
    Configurator.c_test context ~c_flags ~link_flags code |> fun result ->
    Some result
  ...
  let skip_if_android _context k =
    match !Arguments.android_target with Some true -> None | _ -> k ()
        ...
        detect =
          (fun context ->
            let detect_esy_wants_libev () =
              ...
              | _ -> (
                  match
                    (Sys.getenv "LIBEV_CFLAGS", Sys.getenv "LIBEV_LIBS")
                  with
            ...
            let should_look_for_libev =
              ...
              | None -> (
                  ...
                  | None ->
                      (* we're not under esy *)
                      let os =
                        Configurator.ocaml_config_var_exn context "os_type"
                      in
                      os <> "Win32" && !Arguments.android_target <> Some true)
            in

            if not should_look_for_libev then None
            else
              let code =
                {|
          #include <ev.h>
    ...
    feature
      {
        ...
        detect =
          (fun context ->
            if !Arguments.use_pthread = Some false then None
            else
        ...
        |}
              ...
              | Some true -> Some true
              | no -> (
                  if !Arguments.android_target = Some true then no
                  else
    ...
    feature
      {
        ...
        detect =
          (fun context -> compiles context (nanosecond_stat "tim.tv_nsec"));
      }
    ...
    feature
      {
        ...
        detect =
          (fun context -> compiles context (nanosecond_stat "timespec.tv_nsec"));
      }
    ...
    feature
      {
        ...
        detect = (fun context -> compiles context (nanosecond_stat "timensec"));
      }
...
let () =
  (match List.partition (( = ) "--save") (Array.to_list Sys.argv) with
  | [ "--save" ], rest ->
      Configurator.Flags.write_lines "src/unix/discover_arguments"
        [ String.concat " " (List.tl rest) ];
      exit 0
  | _ -> ());

  Configurator.main ~args:Arguments.args ~name:"lwt" (fun context ->
      ...
      Output.write_ml_file
        ~extra:
          [
            { name = "android"; found = !Arguments.android_target = Some true };
            { name = "libev_default"; found = libev_default };
          ]
        macros;
